         ...
         <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
          ...
          <v-btn icon dark @click="dialog = false">
            <v-icon>mdi-close</v-icon>
                                         ...
                                         <v-toolbar-title>Modifier {{plat.tab['nom']}}</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
              <v-col cols="1">
                <app-deleteplat :plat="plat"></app-deleteplat>
              </v-col>
              <v-col cols="1">
              </v-col>
              <v-col cols="1">
                 <app-safeplat :canBeSafe="plat.canBeSafe()" :plat="plat" @save="save" action="modifier"></app-safeplat>
              </v-col>
          </v-toolbar-items>
        </v-toolbar>
                                    <v-container class="grey lighten-5">
                                        <v-row>
                                            ...
                                            <v-col cols="3">
                                                <v-img :src="pictureLink"></v-img>
                                                <v-file-input
                                                        :rules="rules"
                                                        accept="image/png, image/jpeg, image/bmp"
                                                        v-model="tmpFile"
                                                        placeholder="Modifier cette image ? "
...
export default {
    ...
    ,
        uploadImage:function (e) {
            this.plat.tmpFile = e;
            this.pictureLink = URL.createObjectURL(new Blob([this.tmpFile], {type: 'text/plain'}));
        }
    }
    ,
    created: function () {
        ...
        this.platName = this.plat.tab['nom'];
        this.platCategorie = this.plat.tab['categorie'];
        this.platPrix = this.plat.tab['prix'];
        this.solo = this.plat.toString();
        this.max['sauce'] = this.plat.tab['max'].sauce;
        this.max['supplements'] = this.plat.tab['max'].supplements;
        this.pictureLink = this.plat.picture;
        if (this.plat.tab['nom'] == '')
            this.isNew = true;
